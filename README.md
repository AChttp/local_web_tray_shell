...
```text
%LocalAppData%\SwitchShell\logs\switch-日期-时间-pid.log
```

记录启动信息、WebView2 初始化、命令启动/退出/重试、站点加载失败、健康状态变化、托盘菜单开合、退出流程以及未处理异常（含堆栈）。每行即时落盘，程序卡死或崩溃也不会丢日志。也可通过托盘菜单的"打开日志文件夹"直接打开。
